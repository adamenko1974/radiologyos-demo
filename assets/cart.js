...
requestForm?.addEventListener('submit', e => {
  ...
  pushToStaffPanel({
    ...
    desiredDate: dateISO,
    comment: [ref, time !== 'будь-який' ? 'Зручний час: ' + time : '', comment !== '—' ? comment : '', channel === 'call' ? 'Просить зателефонувати' : ''].filter(Boolean).join('. ')
  });
...
function showSuccess(channel, hasFiles) {
  const texts = {
    whatsapp: 'Чат WhatsApp відкрито — надішліть підготовлене повідомлення.' + (hasFiles ? ' Не забудьте вручну прикріпити файли висновку.' : ''),
    viber: 'Відкрито чат Viber. Текст заявки скопійовано — вставте його в повідомлення.' + (hasFiles ? ' Файли висновку прикріпіть вручну.' : ''),
    call: 'Заявку передано в реєстратуру. Ми зателефонуємо вам для підтвердження запису. Стежити за статусом можна в особистому кабінеті.'
  };
